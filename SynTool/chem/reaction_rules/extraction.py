...
def add_environment_atoms(cgr: CGRContainer, center_atoms: Set[int], environment_atom_count: int) -> Set[int]:
    """
    ...
    :param center_atoms: A set of atom id corresponding to the center atoms of the reaction.
    :param environment_atom_count: An integer specifying the depth of the environment around the reaction center to be
    included. If it's 0, only the reaction center is included. If it's 1, the first layer of surrounding atoms
    is included, and so on.

    :return: A set of atom id including the center atoms and their environment atoms up to the specified depth.
     If environment_atom_count is 0, the original set of center atoms is returned unchanged.
    """
...
def add_functional_groups(reaction: ReactionContainer, center_atoms: Set[int],
                          func_groups_list: List[MoleculeContainer]) -> Set[int]:
    """
    ...
    :param center_atoms: A set of atom id corresponding to the center atoms of the reaction.
    :param func_groups_list: A list of functional group objects (MoleculeContainer or QueryContainer) to be considered
    when including functional groups. These objects define the structure of the functional groups to be included.

    :return: A set of atom id corresponding to the rule atoms, including atoms from the specified functional groups if
    include_func_groups is True. If include_func_groups is False, the original set of center atoms is returned.
    """
...
def add_ring_structures(cgr: CGRContainer, rule_atoms: Set[int]) -> Set[int]:
    """
    Adds ring structures to the set of rule atoms if they intersect with the reaction center atoms.

    :param cgr: A condensed graph representation of a reaction (CGRContainer object).
    :param rule_atoms: A set of atom id corresponding to the center atoms of the reaction.

    :return: A set of atom id corresponding to the original rule atoms and the included ring structures.
    """
...
def add_leaving_incoming_groups(reaction: ReactionContainer, rule_atoms: Set[int], keep_leaving_groups: bool,
                                keep_incoming_groups: bool) -> Tuple[Set[int], Dict[str, Set]]:
    """
...
def clean_molecules(rule_molecules: List[MoleculeContainer],
                    reaction_molecules: Tuple[MoleculeContainer],
                    reaction_center_atoms: Set[int],
                    atom_retention_details: Dict[str, Dict[str, bool]]) -> List[QueryContainer]:
    ...
    This dictionary should have two keys: "reaction_center" and "environment", each mapping to another dictionary.
    The nested dictionaries should have keys representing atom attributes (like "neighbors", "hybridization",
    "implicit_hydrogens", "ring_sizes") and boolean values. A value of True indicates that the corresponding attribute
...
def clean_atom(query_molecule: QueryContainer, attributes_to_keep: Dict[str, bool], atom_number: int) -> QueryContainer:
    """
    ...
    representing the attribute names, and the values should be booleans indicating whether to retain (True) or remove
    (False) that attribute. Expected keys are:
...
def create_substructures_and_reagents(reaction: ReactionContainer, rule_atoms: Set[int], as_query_container: bool,
                                      keep_reagents: bool) -> Tuple[List[MoleculeContainer], List[MoleculeContainer], List]:
    """
    ...
    :param reaction: The reaction object (ReactionContainer) from which to extract substructures. This object  represents
    a chemical reaction with specified reactants, products, and possibly reagents.
...
def assemble_final_rule(reactant_substructures: List[QueryContainer], product_substructures: List[QueryContainer],
                        reagents: List, meta_debug: Dict[str, Set], keep_metadata: bool,
                        reaction: ReactionContainer) -> ReactionContainer:
...
def validate_rule(rule: ReactionContainer, reaction: ReactionContainer) -> bool:
    ...
    :param rule: The reaction rule to be validated. This is a ReactionContainer object representing a chemical
    reaction rule, which includes the necessary information to perform a reaction.
    :param reaction: The original reaction object (ReactionContainer) against which the rule is to be validated.
    This object contains the actual reactants and products of the reaction.
...
def create_rule(config: RuleExtractionConfig, reaction: ReactionContainer) -> ReactionContainer:
    ...
    :param config: An instance of ExtractRuleConfig, containing various settings that determine how the rule is created,
    such as environmental atom count, inclusion of functional groups, rings, leaving and incoming groups, and other parameters.
    :param reaction: The reaction object (ReactionContainer) from which to create the rule. This object represents
    a chemical reaction with specified reactants, products, and possibly reagents.

    :return: A ReactionContainer object representing the extracted reaction rule. This rule includes various elements
    of the reaction as specified by the configuration, such as reaction centers, environmental atoms, functional groups, and others.
    """
    ...
    rule_atoms, meta_debug = add_leaving_incoming_groups(reaction,
                                                         rule_atoms,
...
def extract_rules(config: RuleExtractionConfig, reaction: ReactionContainer) -> List[ReactionContainer]:
    """
    Extracts reaction rules from a given reaction based on the specified configuration.

    :param config: An instance of ExtractRuleConfig, which contains various configuration settings for rule extraction,
    such as whether to include multicenter rules, functional groups, ring structures, leaving and incoming groups, etc.
    :param reaction: The reaction object (ReactionContainer) from which to extract rules. The reaction object represents
    a chemical reaction with specified reactants, products, and possibly reagents.

    :return: A list of ReactionContainer objects, each representing a distinct reaction rule. If config.multicenter_rules
    is True, a single rule encompassing all reaction centers is returned. Otherwise, separate rules for each reaction
    center are extracted, up to a maximum of 15 distinct centers.
...
@ray.remote
def process_reaction_batch(batch: List[Tuple[int, ReactionContainer]],
                           config: RuleExtractionConfig) -> List[Tuple[int, List[ReactionContainer]]]:
    """
...
def process_completed_batches(futures: Dict, rules_statistics: Dict, pbar: tqdm, batch_size: int) -> None:
    """
...
def sort_rules(rules_stats: Dict, min_popularity: int,
               single_reactant_only: bool) -> List[Tuple[ReactionContainer, List[int]]]:
    """
    ...
    :type min_popularity: The minimum number of occurrence of the reaction rule to be selected.
...
def extract_rules_from_reactions(config: RuleExtractionConfig, reaction_file: str,
                                 rules_file_name: str, num_cpus: int, batch_size: int) -> None:
