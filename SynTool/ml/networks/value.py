...
class ValueNetwork(MCTSNetwork, LightningModule, ABC):
    ...
    def __init__(self, vector_dim: int, *args: Any, **kwargs: Any) -> None:
        """Initializes a value network, and creates linear layer for predicting
        the synthesisability of given retron represented by molecular graph.

        :param vector_dim: The dimensionality of the output linear
            layer.
        """
    ...
    def forward(self, batch) -> torch.Tensor:
        """Takes a batch of molecular graphs, applies a graph convolution
        returns the synthesisability (probability given by sigmoid function) of
        a given retron represented by molecular graph precessed by graph
        convolution.
    ...
    def _get_loss(self, batch: Batch) -> Dict[str, Tensor]:
        ...
        batch for the retron synthesysability prediction.

        :param batch: The batch of molecular graphs.
        :return: The dictionary with loss value and balanced accuracy of
            retron synthesysability prediction.
        """
